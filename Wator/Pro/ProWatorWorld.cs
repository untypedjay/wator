...
namespace VPS.Wator.Pro
{
    public class ProWatorWorld : IWatorWorld
    {
        ...
        public int InitialTotalPopulation { get; private set; }
        #endregion

        public ProWatorWorld(Settings settings)
        {
            ...
            InitialTotalPopulation = InitialFishPopulation + InitialSharkPopulation;
            ...
            for (int i = 0; i < Width; i++)
            {
                for (int j = 0; j < Height; j++)
                {
                    ...
                    else if (value < InitialTotalPopulation)
                    {
        ...
        private bool IsNeighborValid(Type type, int i, int j)
        {
            if (type.IsInstanceOfType(Grid[i, j]) && (!Grid[i, j].Moved) && (Grid[i, j] != null))
            {
                return true;
            }
            return false;
        ...
        public Point SelectNeighbor(Type type, Point position)
        {
            ...
            int neighborAmount = neighbors.Length;
            if (neighborAmount > 1)
            {
                return neighbors[random.Next(neighborAmount)];  // return random neighbor (prevent bias)
            }
            else if (neighborAmount == 1)
